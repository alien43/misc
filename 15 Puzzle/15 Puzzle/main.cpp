...
private:
    bool needExitBadEstimation() {
        const int SIZE = 6;
        const double MIN_ADDITION = 0.5;
        if (estimations_.size() < SIZE) {
            return false;
        }
        auto it = estimations_.rbegin() + SIZE - 1;
        return *std::max_element(estimations_.rbegin(), it) <= *it + MIN_ADDITION;
    }
    ...
    void stopOnTimeLimit() {
        if (operationsPerformed_ % 1000000 != 0) {
            return;
        }
        auto elapsed = system_clock::now() - startTime_;
        // std::cerr << "Elapsed " << elapsed.count() * system_clock::duration::period::num / system_clock::duration::period::den << " seconds\n";;
...
int main() {
    ...
    for (int T = 0; T < 10; ++T) {
        ...
        std::vector<int> cells = range(16);
        std::rotate(cells.begin(), cells.begin() + 1, cells.end());
        Field field(4, 4, cells);
        for (int i = 0; i < T; ++i) {
            field.moveEmpty(Direction(std::rand() % 4));
        }
